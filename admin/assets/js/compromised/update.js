...
const populateForm = (data) => {
  ...
  data.typeDangerousActions.forEach((type) => {
      const subType = type.subTypes[0];
      accordionBody.innerHTML += `<div class="accordion-item">
                                      <h2 class="accordion-header" id="flush-heading${type.typeId}">
                                          <button class="accordion-button collapsed" type="button" data-bs-toggle="collapse" data-bs-target="#flush-collapse${type.typeId}" aria-expanded="false" aria-controls="flush-collapse${type.typeId}">
                                              ${type.typeName}
                                          </button>
                                      </h2>
                                      <div id="flush-collapse${type.typeId}" class="accordion-collapse collapse" aria-labelledby="flush-heading${type.typeId}" data-bs-parent="#accordionFlushExample">
  ...
  const tindakLanjutTextarea = document.getElementById('exampleFormControlTextarea1');
...
const updateCompromised = async (event, reportid) => {
  ...
  const targetURL = 'https://asia-southeast2-gis-project-401902.cloudfunctions.net/updatedatacompromised';
  ...
  const requestOptions = {
      ...
      body: JSON.stringify({
          ...
          immediateAction: document.getElementById('tindakanPerbaikanSegera').innerText,
  ...
  try {
      ...
      if (data.status === true) {
          ...
          window.location.href = 'https://portsafe-apps.github.io/admin/compromisedreport.html';
      } else {
          showAlert(data.message, 'error');
      }
  } catch (error) {
...
document.getElementById('updateButton').addEventListener('click', (event) => updateCompromised(event, compromisedIdFromURL));
