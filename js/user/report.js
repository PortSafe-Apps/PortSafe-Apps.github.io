...
const displayDetailedReport = async (reportid, detailContainerId) => {
    ...
    if (data.status === 200) {
      ...
      detailCard.innerHTML = `
        <div class="card-body">
          ...
          <p>${detailContainer.reportid}</p>
          
          <h6 class="mb-0">Tanggal Pelaporan</h6>
          <p>${detailContainer.date}</p>
          
          <h6 class="mb-0">Informasi Pengawas</h6>
          <p class="mb-0">${detailContainer.user.nama}</p>
          <p>${detailContainer.user.jabatan}</p>
          
          <h6 class="mb-0">Lokasi Kejadian</h6>
          <p>${detailContainer.location.locationName}</p>
          
          <h6 class="mb-0">Deskripsi Pengamatan</h6>
          <p>${detailContainer.description}</p>
          
          <h6>Foto Kejadian</h6>
          <div class="text-center">
            <img class="w-75 mb-4" src="${detailContainer.observationPhoto}" alt="Foto Kejadian">
          </div>
          
          <h6 class="mb-0">Tindakan Berbahaya yang Dilakukan</h6>
          <ul class="ps-0 fs-6">
            ${detailContainer.typeDangerousActions.map((action, index) => `
          ...
          <p>${detailContainer.area.areaName}</p>
  
          <h6 class="mb-0">Tindakan Perbaikan Segera</h6>
          <p>${detailContainer.immediateAction}</p>
  
          <h6>Foto Tindakan Perbaikan</h6>
          <div class="text-center">
            <img class="w-75 mb-4" src="${detailContainer.improvementPhoto}" alt="Foto Tindakan Perbaikan">
          </div>
  
          <h6 class="mb-0">Tindakan Pencegahan Terulang Kembali</h6>
          <p>${detailContainer.correctiveAction}</p>
        </div>
      `;
