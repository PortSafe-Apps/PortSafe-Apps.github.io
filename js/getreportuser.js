...
// Fungsi untuk mendapatkan semua data report dari server
const getAllReportData = async () => {
  ...
  try {
      ...
      if (data.status === 200) {
          // Panggil fungsi untuk menampilkan semua data ke dalam card
          displayReportData(data.data, 'reportContainer');

          // Panggil fungsi untuk menampilkan satu data terbaru ke dalam card
...
// Fungsi untuk menampilkan seluruh data report dalam card
...
// Fungsi untuk menampilkan satu data terbaru dalam card
const displayLatestReport = (reportData, latestCardContainerId) => {
  ...
  if (reportData && reportData.length > 0) {
      ...
      // Konversi tanggal
      const latestReportDate = new Date(latestReport.date);
      const formattedLatestDate = latestReportDate.toLocaleDateString();
      ...
      newCard.innerHTML = `
        <div class="card-body">
          <div class="d-flex justify-content-between">
            ...
            <div class="timeline-text mb-2">
              <span class="badge mb-2 rounded-pill bg-dark">${formattedLatestDate}</span>
            </div>
          </div>
          <div class="divider mt-0"></div>
          <div class="text-content mb-2">
...
// Panggil fungsi untuk mendapatkan dan menampilkan data
getAllReportData();
