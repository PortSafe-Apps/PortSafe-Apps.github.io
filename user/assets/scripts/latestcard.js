...
function getTokenFromCookies(cookieName) {
  ...
  const latestDisplayReportData = (reportData, cardContainerId) => {
    ...
    if (reportData && reportData.length > 0) {
      ...
      const badgeCategory =
        latestReport && latestReport.category
          ? latestReport.category === "Unsafe Action"
            ? "danger"
      ...
      const categoryBadge = `<span class="badge bg-${badgeCategory} text-white font-10 mb-1 d-block rounded-s">
          <i class="fa ${badgeIcon}"></i> ${latestReport.category}
        </span>`;
      ...
      const locationName = latestReport.location
      ...
      const userName =
        (latestReport.user && latestReport.user.nama) ||
        "Pengguna Tidak Diketahui";
  
      // Sesuaikan struktur kartu dengan data yang Anda miliki
      const newCard = document.createElement("div");
      newCard.className = "card card-style mb-3";
      newCard.id = `card-${latestReport.category.toLowerCase()}-${
        latestReport.index + 1
      }`;
